...
//     on Chrome used to stay WebM and play as silence on an iPhone), the audio
//     layout follows the source's own facts (one channel, or a clip inside the
//     5-minute recording cap -> mono 96k; longer/stereo audio -> stereo 128k),
//     while MP3/AAC/WAV/FLAC and MP4/MOV keep the ordinary 8% rule — proved on
//     real generated bytes, including that the moov atom lands before the audio
//     data.
//
// The plumbing around it (the compatibility queue, the serving gate, the ledger,
// the bucket sweep) is covered end-to-end by scripts/test-upload-pipeline.js;
// this file is the coverage contract itself, so it runs anywhere and only needs
// ffmpeg + ffprobe (plus heif-convert for the HEIC half, which the app image
// installs).
//
...
async function main() {
  ...
  try {
    ...
    console.log('\n-- the compatibility set: what the queue repairs, and what it leaves to the sweep --');
    // The queue that runs on its own is for bytes a reader's platform cannot open
    // AT ALL (`normalize` in planFor's terms). Everything playable is ordinary
    // shrinking, which the scheduled bucket sweep owns — so a file the reader was
    // just handed is not rewritten underneath them. These two checks are the
    // contract between those two paths.
    check('ordinary playable media is NOT a repair job',
      ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.mp4', '.mov', '.m4v', '.mp3', '.m4a', '.aac', '.wav', '.flac']
        .every((e) => !media.COMPATIBILITY_EXTS.has(e) && !media.needsCompatibility('files/x' + e)));
    check('the Apple-unplayable families ARE (Opus/Vorbis audio, non-MP4 video)',
      ['.webm', '.ogg', '.oga', '.opus', '.weba', '.mkv', '.avi', '.wmv', '.flv', '.3gp', '.mpg', '.wma', '.amr']
        .every((e) => media.COMPATIBILITY_EXTS.has(e) && media.needsCompatibility('files/x' + e)));
    check('a HEIC is a repair job too (no Windows browser can open one)',
      media.COMPATIBILITY_EXTS.has('.heic') && media.COMPATIBILITY_EXTS.has('.heif') && media.needsCompatibility('files/x.HEIC'));
    check('the extension is matched case-insensitively',
      media.needsCompatibility('files/MY-CLIP.WEBM') === true && media.needsCompatibility('files/photo.JPG') === false);
    ...
    if (!heif) {
      ...
      check('a HEIC is a candidate the compressor will take on',
    ...
    for (const p of ['jpeg', 'png', 'webp', 'gif', 'mp4', 'mp3', 'm4a', 'm4a-mono', 'wav2mp3']) {
      let ok = true;
    ...
    // index first, ONE channel at 96k (not the duplicated stereo 128k the m4a
    // plan used to force on it), and it is PUBLISHED even though AAC is the
    // bigger file — which is the decision that used to keep the WebM.
    console.log('\n-- bytes: a WebM/Opus voice note comes out the way an iPhone plays it --');
    // White noise, not a tone: ffmpeg's AAC encoder undershoots its bitrate
    // target on trivial content (a sine can come out SMALLER at 128k stereo than
    // at 96k mono, which says nothing about real speech), so the byte comparison
    // below needs a signal the encoder has to actually spend bits on.
    const voice = f('voice.webm');
    const longClip = f('long.webm');
    const stereoClip = f('noise-stereo.webm');
    const monoOk = gen(['-f', 'lavfi', '-i', 'anoisesrc=color=white:duration=2', '-ac', '1', '-c:a', 'libopus', '-b:a', '24k'], voice);
    const stereoOk = monoOk && gen(['-f', 'lavfi', '-i', 'anoisesrc=color=white:duration=2', '-ac', '2', '-c:a', 'libopus', '-b:a', '96k'], stereoClip);
    // Longer than the composer's 5-minute voice-message cap, so it must NOT be
    // treated as one — 301s of silence encodes in a moment at 6k.
    const longOk = monoOk && gen(['-f', 'lavfi', '-i', 'anullsrc=r=48000:cl=stereo', '-t', '301', '-c:a', 'libopus', '-b:a', '6k'], longClip);
    if (!monoOk) {
      console.log('  note  this ffmpeg has no libopus encoder — the voice-note case is not exercised here');
    } else {
      const raw = media.planFor('audio/webm', 'files/voice.webm');
      check('a Chrome voice note is planned for AAC/MP4 at any size',
        !!raw && raw.pipeline === 'm4a' && raw.outExt === '.m4a' && raw.normalize === true, JSON.stringify(raw));
      check('...and the plan asks the bytes what they are before choosing a layout', raw.probeAudio === true, JSON.stringify(raw));
      const vPlan = await media.resolvePlan(raw, voice);
      check('a one-channel voice note resolves to the mono 96k pipeline',
        vPlan.pipeline === 'm4a-mono', JSON.stringify(vPlan));
      const vOut = f('voice-out.m4a');
      const r = await media.encodeCandidate(vPlan, voice, vOut, 'test');
      const st = fs.existsSync(vOut)
        ? JSON.parse(String(spawnSync('ffprobe', ['-v', 'error', '-show_entries', 'stream=codec_name,channels', '-show_entries', 'format=format_name', '-of', 'json', vOut], { encoding: 'utf8' }).stdout || '{}'))
        : {};
      ...
      check('...as ONE channel, not the same channel written twice',
        r.ok && Number(stream.channels) === 1, JSON.stringify({ channels: stream.channels }));
      // The other side of the rule: a clip longer than the composer's own
      // recording cap is not a voice message, so the full stereo layout stands —
      // and the stereo pipeline itself really does write two channels.
      if (longOk) {
        const lPlan = await media.resolvePlan(raw, longClip);
        check('a clip longer than the 5-minute voice cap keeps the stereo 128k pipeline',
          lPlan.pipeline === 'm4a' && !lPlan.mono, JSON.stringify(lPlan));
      }
      if (stereoOk) {
        const sOut = f('stereo-out.m4a');
        const sr = await media.encodeCandidate({ pipeline: 'm4a', outExt: '.m4a' }, stereoClip, sOut, 'test');
        const sCh = sr.ok ? JSON.parse(String(spawnSync('ffprobe', ['-v', 'error', '-show_entries', 'stream=channels', '-of', 'json', sOut], { encoding: 'utf8' }).stdout || '{}')).streams : null;
        check('...and the stereo pipeline writes two channels',
          sr.ok && Number(((sCh || [])[0] || {}).channels) === 2, JSON.stringify({ ok: sr.ok, err: sr.error, streams: sCh }));
        const sBytes = fs.existsSync(sOut) ? fs.statSync(sOut).size : 0;
        check('...which is the bigger file (the mono voice note is the saving)',
          outBytes > 0 && sBytes > outBytes, JSON.stringify({ mono96: outBytes, stereo128: sBytes }));
      }
    }
