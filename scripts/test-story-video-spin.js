...
//
// The fix: storyThumbSpin(host, media) hides the <video> behind a spinner
// (the .st-vid-loading class) until `loadeddata`, or until storyThumbRetry
// gives up and removes the media — wired in spHero for the hero backdrop and
// for the badge ring thumbnail. Photos are untouched.
//
// Static half (always runs): the helper exists, spHero wires it for both
// surfaces, and the stylesheet hides the video and paints the spinner.
//
// Chrome half (skips without Chrome): runs the REAL spHero sliced out of
// public/js/stories.js against the REAL stylesheet with a stub storyThumbEl
// that builds a genuine (src-less, never-loading) <video>, and asserts the
// loading class is on, the video is really hidden, the spinner pseudo-element
// is animated, `loadeddata` settles it, removing the video (the give-up path)
// settles it, and a photo never arms it.
//
...
check(/storyThumbSpin\(av, thumb\)/.test(heroSrc), 'spHero arms the spinner on the badge ring thumbnail');

console.log('\n[2] the stylesheet hides the video and paints the spinner');
check(/\.st-vid-loading video\{visibility:hidden\}/.test(css), 'a loading thumbnail keeps its <video> hidden');
check(/\.st-vid-loading::after\{[^}]*animation:up-spin/.test(css), 'the spinner is the app\'s own up-spin mark');
check(/\.sp-hero-badge \.st-vid-loading::after\{[^}]*width:18px/.test(css), 'the badge ring gets the small spinner');
...
<div id="hero"></div><div id="hero2"></div><div id="hero3"></div>
<script>
...
window.__t1 = {
  ...
  badgeLoading: badgeAv && badgeAv.classList.contains('st-vid-loading'),
...
async function main() {
  ...
  try {
    ...
    console.log('\n[3] a loading video hero wears the spinner, not the grey placeholder');
    const t1 = await ev('window.__t1');
    check(t1.heroLoading, 'the hero arms .st-vid-loading while the video has no frame');
    check(t1.badgeLoading, 'the badge ring arms it too');
    check(t1.vidHidden, 'the <video> is really hidden (no grey play button can paint)');
