...
- **Desktop** (Windows/macOS/Linux) — WebView2 on Windows (preinstalled),
  ...
  - **Tray icon** — left-click toggles the window open/closed (right-click menu:
    open app, current game, start-on-login toggle, start-minimized toggle,
    quit). Unread notifications put a red dot on the icon's corner (plus the
    Windows taskbar overlay dot / macOS dock badge). On GNOME there is no
  ...
  - **Start on login** — Task Scheduler entry on Windows, LaunchAgent on macOS,
    XDG autostart entry on Linux. Login launches pass `--autostart`; with
    **Start minimized** on (the default) they stay tray-only, otherwise the
    window opens like a manual launch. The toggle sits next to start-on-login
    in the tray menu and in Settings → Desktop app, and is greyed out while
    start-on-login is off. The preference is stored in `settings.json` in the
    app data dir.
...
- The webview loads the remote site, so the frontend can only reach Rust
  through `window.__TAURI__.core.invoke` when `app.withGlobalTauri` is true, and
  Tauri only allows remote-origin calls to commands granted in
  `src-tauri/capabilities/default.json`. App-command permissions are
  autogenerated from the `AppManifest::commands` list in `build.rs`
  (`src-tauri/permissions/autogenerated/`); add a new command there, to the
  capability, and keep frontend calls `.catch()`-guarded for older builds.
