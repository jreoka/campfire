...
function messageEl(m, opts = {}) {
  ...
  } else if (m.content && !m.viewOnce) {
    // A view-once caption is part of the story, not the chat: it renders under
    // the media in the one-shot viewer (like a story caption) instead of as a
...
// A conversation opens on its newest page (20) and stops there. Scrolling up
// near the top pages the next-older 20 in, until the server answers short.
...
const HIST_PAGE = 20;          // messages per older page
