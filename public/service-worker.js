...
self.addEventListener('activate', (e) => {
  e.waitUntil(
    caches.keys().then((keys) => Promise.all(keys.filter((k) => k !== CACHE).map((k) => caches.delete(k)))).then(() => self.clients.claim()).then(async () => {
      // nudge open tabs to check for a new version (they decide whether to reload)
      const wins = await self.clients.matchAll({ type: 'window' });
      wins.forEach((w) => { try { w.postMessage({ t: 'SW_UPDATED' }); } catch {} });
    })
  );
});
